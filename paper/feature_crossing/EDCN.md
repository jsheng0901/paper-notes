...
![模型综合结构](./pics/EDCN/EDCN_1_模型综合结构.png)
...
![模型结构](./pics/EDCN/EDCN_4_模型结构.png)
...
![融合方式对比](./pics/EDCN/EDCN_4.1_融合方式对比.png)
...
- 假设第 l 个cross layer 和第 l 个deep layer 的输出记作 x_l 和 h_l，bridge module 可以表示为 f_l = f(x_l, h_l)，其中 f(⋅) 是一个预定义的交互函数，它以两个向量作为输入并输出一个具有相同维度的向量。具体的交互函数本文尝试了一下四种：
  ...
  - ![attention权重计算方式](./pics/EDCN/EDCN_4.1_attention权重计算方式.png)
...
- 最终网络 b 的regulated representation e_b 为：
  - ![门控输出表达式](./pics/EDCN/EDCN_4.2_门控输出表达式.png)
...
![实验性能对比](./pics/EDCN/EDCN_5.2_实验性能对比.png)
...
![特征heatmap](./pics/EDCN/EDCN_5.4_特征heatmap.png)
...
## 5.6 模型复杂度分析
![模型复杂度对比](./pics/EDCN/EDCN_5.6_模型复杂度对比.png)

## 6 ONLINE A/B TESTING
![线上实验对比](./pics/EDCN/EDCN_6_线上实验对比.png)
