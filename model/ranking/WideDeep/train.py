...
from notes.ranking.WideDeep.model import WideAndDeepModel
from notes.utils.dataloader import create_dataset
from config import WideAndDeepModelConfig
...
def train(config):
    ...
    model = WideAndDeepModel(field_dims, embed_dim, mlp_dims, dropout).to(device)
    ...
    logging.info(f"Wide Deep model test_loss:  {test_loss:.5f} | test_auc: {test_auc:.5f}")
