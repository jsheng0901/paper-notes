...
from notes.ranking.FiBiNet.config import FiBiNETModelConfig
from notes.ranking.FiBiNet.model import FiBiNETModel
...
def train(config):
    ...
    model = FiBiNETModel(field_dims, embed_dim, mlp_dims, dropout, reduction_ratio, bilinear_type).to(device)
    # create optimizer and loss function
    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate, weight_decay=regularization)
    criterion = nn.BCELoss()

    # start train FiBiNET model
    ...
    logging.info(f"FiBiNET model test_loss:  {test_loss:.5f} | test_auc: {test_auc:.5f}")

    return


if __name__ == '__main__':
    logging.info('Start FiBiNET Model Train')
    fibinet_config = FiBiNETModelConfig.all_config
    train(fibinet_config)
