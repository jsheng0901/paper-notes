...
## 2.1 传统推荐模型的演化关系图
![传统推荐模型的演化关系图](./pics/chapter2/2.1_传统推荐模型的演化关系图.png)
...
![基于用户的协同过滤推荐过程](./pics/chapter2/2.2.1_UserCF例子.png)
...
![给予用户的CF排序打分公式](./pics/chapter2/2.2.3_排序打分公式.png)
...
![基于物品的排序打分方式](./pics/chapter2/2.2.4_基于物品的排序打分方式.png)
...
![UserCF与ItemCF的对比分析](./pics/chapter2/2.2.5_CF对比.png)
...
![矩阵分解过程例子](./pics/chapter2/2.3.1_矩阵分解过程例子.png)
...
- 基于用户矩阵U和物品矩阵V，用户u对物品i的预估评分用公式表达：
  ![预估评分公式](./pics/chapter2/2.3.1_预估评分公式.png)
- 其中P_u是用户u在用户矩阵U中对应的行向量，q_i是物品i在物品矩阵V中对应的列向量. 也就是有P_u -> [1, k] * q_i -> [k, 1] = r_u_i -> [1, 1]

## 2.3.2 矩阵分解的求解过程
矩阵分解主要有三种求解的方法。如下：
![常见的矩阵分解方式对比](./pics/chapter2/2.3.2_矩阵分解方式.png)

对比可知，矩阵分解主要是梯度下降来实现。详细步骤如下：
- 确定目标函数，这里目标函数的目的是让公式预测值和实际值的差尽量小，这样才能最大限度的保存共现矩阵的原始信息。
  ![2.3.2_矩阵分解的目标函数](./pics/chapter2/2.3.2_矩阵分解的目标函数.png)
- 对目标函数求偏导，求取梯度下降的方向和幅度。
  - q_i偏导
    ![q_i偏导](./pics/chapter2/2.3.2_q_i偏导.png)
  - p_u偏导
    ![p_u偏导](./pics/chapter2/2.3.2_p_u偏导.png)
...
![加入偏差的评分公式](./pics/chapter2/2.3.3_加入偏差的评分公式.png)
其中μ是全局偏差常数，bi是物品偏差系数，可使用物品i收到的所有评分的均值，bu是用户偏差系数，可使用用户u给出的所有评分的均值。相对应的目标函数也需要更新。
![加入偏差的目标函数](./pics/chapter2/2.3.3_加入偏差的目标函数.png)
...
![矩阵分解章节思维导图](./pics/chapter2/2.3.4_矩阵分解章节思维导图.png)
...
![逻辑回归章节思维导图](./pics/chapter2/2.4.4_逻辑回归章节思维导图.png)
...
![数学公式](./pics/chapter2/2.5.1_数学公式.png)
...
![数学公式](./pics/chapter2/2.5.2_数学公式.png)
...
![数学公式](./pics/chapter2/2.5.3_数学公式.png)

## 2.5.4 从 POLY2 到 FFM 的模型演化过程
- POLY2 模型，图中每个彩色原点代表一个特征交叉项。
  ![poly2模型示意图](./pics/chapter2/2.5.4_poly2模型示意图.png)
- FM 模型，图中每个特征交叉项不再是单独一个圆点，而是3个彩色圆点的内积，代表每个特征有一个3维的隐向量。
  ![FM模型示意图](./pics/chapter2/2.5.4_FM模型示意图.png)
- FFM 模型，图中每个特征都有2个隐向量，根据特征交叉对象特征域的不同，选择使用对应的隐向量。
  ![FFM模型示意图](./pics/chapter2/2.5.4_FFM模型示意图.png)
...
![FM章节思维导图](./pics/chapter2/2.5.4_FM章节思维导图.png)
...
![模型结构](./pics/chapter2/2.6.1_模型结构.png)

## 2.6.2 GBDT 进行特征转换的过程
如图所示，简单说就是一个样本通过所有树之后，落在每棵树的叶子结点的0-1向量的拼接。
![GBDT生成特征向量的过程](./pics/chapter2/2.6.2_特征转换的过程.png)
...
![GBDT+LR章节思维导图](./pics/chapter2/2.6.3_GBDT+LR章节思维导图.png)
...
![LS-PLM模型目标函数](./pics/chapter2/2.7.1_LS-PLM模型目标函数.png)
...
![LS-PLM章节思维导图](./pics/chapter2/2.7.3_LS-PLM章节思维导图.png)

## 2.8 总结一一深度学习推荐系统的前夜
下表为传统的推荐模型对比总结。
![传统的推荐模型对比总结](./pics/chapter2/2.8_总结.png)
